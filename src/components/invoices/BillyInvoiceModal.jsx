...
const BillyInvoiceModal = ({ isOpen, onClose, onOpenCreateCustomer }) => {
  ...
  const currencyList = Array.isArray(currenciesRes?.data) ? currenciesRes.data
    : Array.isArray(currenciesRes) ? currenciesRes : [];

  const deadlineList = Array.isArray(deadlinesRes?.data) ? deadlinesRes.data
    : Array.isArray(deadlinesRes) ? deadlinesRes : [];

  const vatList = Array.isArray(vatRes?.data) ? vatRes.data
    : Array.isArray(vatRes) ? vatRes : [];

  const approveButtonsList = Array.isArray(actionButtonsRes?.data) ? actionButtonsRes.data
    : Array.isArray(actionButtonsRes) ? actionButtonsRes : [];
    ...
    <Modal
      ...
      title={
        <Space>
          ...
          <div>
            <div style={{ fontWeight: 700 }}>Create &amp; Approve Invoice</div>
            <div style={{ fontSize: 12, color: '#94a3b8', fontWeight: 400 }}>
              Saves complete invoice data to POST /api/invoice
            </div>
          </div>
        </Space>
      }
            ...
            label={
              <Space style={{ width: '100%', justifyContent: 'space-between' }}>
                ...
                {hasCustomers && onOpenCreateCustomer && (
                  <Button type="link" size="small" onClick={onOpenCreateCustomer} style={{ padding: 0, fontSize: 11 }}>
                    + Create New
                  </Button>
                )}
              </Space>
